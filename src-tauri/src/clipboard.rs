...
//!
//! # Capabilities: none was added FOR THE CLIPBOARD, and here is why - both halves
//!
//! **The half that is true, and was the right call.**
//! No capability of this application names the clipboard plugin - not
//! `default.json`, which grants Tauri's `core:default` and this crate's own
//! `allow-describe-displays`, and not `veil.json`, which grants three
//! application permissions and no `core:` or plugin one at all. The plugin's
...
//! None of that said anything about THIS crate's own `#[tauri::command]`s -
//! `veil_selected` in particular, which is what calls [`copy_selection`]. Those
//! carry no `plugin:` prefix, and for as long as the application declared no ACL
//! manifest the check simply never ran on them from a local origin: nothing at
//! all stopped `main` from reaching the clipboard THROUGH `veil_selected`
//! rather than through the plugin.
//!
//! **Both halves now hold.** Since 4 September 2026 `src-tauri/permissions/`
//! declares one permission per command, so the application manifest exists and
//! the same ACL check runs on `veil_selected` as on the plugin's own commands;
//! `capabilities/veil.json` grants it to the veil window and to nothing else,
//! and `capabilities/default.json` deliberately does not list it. The webview
//! label check in `ipc.rs` stays in front of it - `ipc.rs` gives the three
//! reasons, of which the URI scheme is the one no capability can cover.
//!
//! Read from the plugin's source, its permission files and the vendored
//! `tauri` 2.11.5 source, and checked against the generated ACL by
//! `ipc::tests::the_acl_grants_each_command_to_its_own_window_and_to_no_other`;
//! NOT confirmed by a run of the application.
